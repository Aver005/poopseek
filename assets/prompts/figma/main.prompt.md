...
## Дизайн-токены — используй имена, а не значения

В этом проекте есть тема со следующими токенами:

```
{{TOKENS_TABLE}}
```

**Правило: для цветов и для размеров (gap / padding / radius) предпочитай токен-имя сырому значению.**
...
Эти ссылки превратятся в Figma-переменные — изменение токена в палитре прокатится на все ноды автоматически.

**Когда сырое значение оправдано:** один-в-один цвет/размер, не имеющий смысла в палитре (`fill="#FF00FF"` на debug-плашке, разовые `gap={3}`). Тогда пиши hex / число — компилятор оставит как есть.
