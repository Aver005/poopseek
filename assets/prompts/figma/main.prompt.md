...
<Frame as="button-primary">
  <Text variant="body">Submit</Text>
</Frame>
```

`as` подставляет дефолтные пропсы из `components` секции DESIGN.md (`backgroundColor` / `textColor` / `rounded` / `padding` / `typography`). Любой явно заданный пропс на ноде перебивает дефолт компонента.
