// jsx-spec.ts — ПОЛНАЯ ВЕРСИЯ С ЭКСПОРТАМИ
...
// ============================================
// СПЕЦИФИКАЦИЯ КОМПОНЕНТОВ — ЭКСПОРТИРУЕМАЯ ЧАСТЬ
// ============================================
...
const FRAME_PROPS = new Set([
    ...
    "gap", "pad", "padX", "padY", "padTop", "padRight", "padBottom", "padLeft",
...
// ============================================
// ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (из classname.ts)
// ============================================
...
function parseArbitrarySize(token: string): { axis: "w" | "h" | "fontSize"; value: number } | null
{
    // w-[...] / h-[...]
    const sizeMatch = /^(?:min-|max-)?(w|h)-\[(\d+(?:\.\d+)?)(px|rem|em)?\]$/.exec(token);
    if (sizeMatch) {
        let value = parseFloat(sizeMatch[2]!);
        const unit = sizeMatch[3] ?? "px";
        if (unit === "rem" || unit === "em") value *= 16;
        return { axis: sizeMatch[1] as "w" | "h", value: Math.round(value) };
    }

    // text-[...] — arbitrary font size
    const textMatch = /^text-\[(\d+(?:\.\d+)?)(px|rem|em)?\]$/.exec(token);
    if (textMatch) {
        let value = parseFloat(textMatch[1]!);
        const unit = textMatch[2] ?? "px";
        if (unit === "rem" || unit === "em") value *= 16;
        return { axis: "fontSize", value: Math.round(value) };
    }
...
export function resolveClassNameProps(className: string): ClassNameProps
{
    ...
    for (const token of tokenizeClassName(className))
    {
        ...
        if (arbitrarySize !== null)
        {
            ...
            else if (arbitrarySize.axis === "fontSize") result.fontSize = arbitrarySize.value;
            continue;
        }

        // Отрицательные margin: -m-*, -mt-*, -mr-*, -mb-*, -ml-*, -mx-*, -my-*
        if (token.startsWith("-m-")) {
            const value = -readScaleValue("m-", token.slice(1));
        ...
        if (token.startsWith("-mt-")) { result.marginTop = -readScaleValue("mt-", token.slice(1)); continue; }
        if (token.startsWith("-mr-")) { result.marginRight = -readScaleValue("mr-", token.slice(1)); continue; }
        if (token.startsWith("-mb-")) { result.marginBottom = -readScaleValue("mb-", token.slice(1)); continue; }
        if (token.startsWith("-ml-")) { result.marginLeft = -readScaleValue("ml-", token.slice(1)); continue; }
        if (token.startsWith("-mx-")) {
            const value = -readScaleValue("mx-", token.slice(1));
            result.marginLeft = value;
            result.marginRight = value;
            continue;
        }
        if (token.startsWith("-my-")) {
            const value = -readScaleValue("my-", token.slice(1));
            result.marginTop = value;
            result.marginBottom = value;
            continue;
        }
